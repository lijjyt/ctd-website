  ...
  try {
    ...
    const html = `
        <div class="daily-link">
          <a href="daily.html">View Daily Forecast</a>
            </div>
    `;
    // Set the HTML content of the weather container
...
// Function to populate the daily forecast on the daily.html page
async function getDailyForecast() {
  try {
    if (globalCoordinates) {
      const response = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${globalCoordinates.lat}&longitude=${globalCoordinates.lng}&daily=weathercode,temperature_2m_max,temperature_2m_min&timezone=auto`);
      const data = await response.json();
      ...
      const headerWeatherCell = document.createElement('th');
      headerWeatherCell.textContent = 'Weather';
      headerRow.appendChild(headerWeatherCell);

      table.appendChild(headerRow);

      // Iterate over the daily forecast data
      for (const dayData of dailyForecast) {
        ...
        weatherCell.textContent = weatherDesc;
